...
export const episodeController = {
    async create(request: FastifyRequest<{ Params: { seriesID: string } }>, reply: FastifyReply) {
        ...
        try {
            const { title, description, durationInMinutes, season, episodeNumber, image } = validation.data;

            if (image) {
                const base64Data = image.replace(/^data:image\/\w+;base64,/, "");
                require("fs").writeFile(`public/posters/episode/${title}.png`, base64Data, "base64");
            }
            ...
            await prisma.episode.create({
                data: {
                    ...
                    season,
                    episodeNumber,
                    image: image ? `/posters/episode/${title}.png` : null,
                    series: {
                        connect: {
            ...
            return reply.status(201).send({ success: true, message: "Episode created" });
    ...
    async update(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) {
        ...
        try {
            ...
            return reply.status(200).send({ success: true, message: "Episode updated" });
    ...
    async delete(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) {
        ...
        try {
            ...
            if (episode.image) {
                require("fs").unlinkSync(path.join(__dirname, `../../public${episode.image}`));
            }
            ...
            return reply.status(200).send({ success: true, message: "Episode deleted" });
