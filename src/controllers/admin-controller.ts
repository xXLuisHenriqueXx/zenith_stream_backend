...
export const adminController = {
  async register(request: FastifyRequest, reply: FastifyReply) {
    ...
    if (accessKey !== env.ACCESS_KEY) {
      return reply
        .status(401)
        .send({ success: false, message: "Invalid access key" });
    }
    ...
    try {
      ...
      reply.setCookie(
        "token",
        cookieService.createCookie(email, "ROLE_ADMIN"),
        {
          path: "/",
          httpOnly: true,
          sameSite: "lax",
  ...
  async login(request: FastifyRequest, reply: FastifyReply) {
    ...
    try {
      ...
      reply.cookie(
        "token",
        cookieService.createCookie(admin.email, admin.role),
        { httpOnly: true }
      );

      return reply
        .status(201)
        .send({ success: true, message: "Admin logged in successfully" });
    } catch (error: unknown) {
      console.error(error);
      return reply
        .status(500)
